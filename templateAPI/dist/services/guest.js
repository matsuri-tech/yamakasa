...
exports.GuestJourneyEvent = exports.GuestJourneyPhase = exports.GuestAttribute = void 0;
// ゲスト情報を取得するクラス
class GuestAttribute {
    constructor(nationality, confirmation_code) {
        this.listing_id = null;
        this.nationality = nationality;
        this.confirmation_code = confirmation_code;
    }
    // nationality を選択するためのメソッド
    static resolveNationality(nationalities) {
    ...
    static async get_listing_id(confirmation_code, bigQueryUtility, nationality) {
...
exports.GuestAttribute = GuestAttribute;
// 今日の日付からphaseと各ポイントから何日離れているのかを計算するクラス
class GuestJourneyPhase {
    constructor(confirmation_code) {
    ...
    // SQLで日付を取得するメソッド
    async getDatesFromBQ(confirmation_code, bigQueryUtility) {
        const query = `
    ...
    setStatus(today, status_precheckin) {
    ...
    // ステータスのT/F判定を行う
    isStatusActive(eventDate, today) {
        return new Date(eventDate).getTime() <= new Date(today).getTime();
    }
    // GuestJourneyPhaseのデータをBigQueryから取得し、プロパティを設定する静的メソッド
    static async fetchGuestJourneyData(confirmation_code, bigQueryUtility, status_precheckin) {
...
exports.GuestJourneyPhase = GuestJourneyPhase;
//トラブル、レビュー、清掃遅延、事前予約の情報を取得するクラス
class GuestJourneyEvent {
    constructor(status_precheckin, status_review, guest_review_submitted_at, confirmation_code, 
    // 受け取るときも単体オブジェクトに
    trouble_genre_user = { genre: [], user: [] }) {
    ...
    static async fetchGuestJourneyEventData(confirmation_code, guest_review_submitted_at, status_precheckin, status_review, bigQueryUtility) {
        // trouble_genre_user はひとまずデフォルト値にしておく（空のgenre/user）
        const guestJourneyEvent = new GuestJourneyEvent(status_precheckin, status_review, guest_review_submitted_at, confirmation_code, { genre: [], user: [] });
        try {
    ...
    async fetchTroubleGenreUserData(bigQueryUtility) {
        try {
            const troubleQuery = `
    ...
    // SQLでarrived_atを取得する
    async getArrivedAt(bigQueryUtility) {
        const query = `
      SELECT arrived_at
      FROM (
        SELECT 
    ...
    async checkCleaningDelay(bigQueryUtility) {
        const query = `
    ...
    calculateDaysFromDate(eventDate) {
        ...
        return Math.floor((new Date(this.today_date).getTime() - new Date(eventDateOnly).getTime()) /
            (1000 * 60 * 60 * 24));
    }
}
exports.GuestJourneyEvent = GuestJourneyEvent;
